...
| **Orchestrator progress** | After each successful agent call, `reportProgress(expectedPhase)` runs: read STATE.md, call optional `onProgress(snapshot)`, log **warning** if `snapshot.phaseNumber !== expectedPhase` (informational only, does not fail the run). |
