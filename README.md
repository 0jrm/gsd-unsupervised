...
**Status server and dashboard API:** Use `--status-server <port>` to enable the HTTP status server (e.g. `./bin/unsupervised-gsd --goals goals.md --status-server 3000`). The daemon starts the server before the goal loop and closes it on shutdown (SIGINT/SIGTERM). Endpoints: **GET /** and **GET /status** — legacy JSON `{ running, currentGoal?, phaseNumber?, planNumber?, heartbeat? }`. **GET /api/status** — dashboard JSON with same fields plus `currentAgentId`, `stateSnapshot`, `sessionLogEntries`, `gitFeed`, and placeholder `tokens`/`cost`. Use from a browser or `curl` on the same machine or from another device on the same network. **Heartbeat:** While the agent runs, `.planning/heartbeat.txt` is updated every 15s; if it’s missing or older than 60s with a `running` session, the next startup treats it as a crash and can resume.
