...
**Purpose:** Persist milestone discussion context so `start a new chat` can be used between commands. This file is consumed by `/gsd/new-milestone` and deleted after the milestone is created.
...
The file exists only to pass context from `discuss-milestone` to `create-milestone` across a `start a new chat` boundary.

**Lifecycle:**
1. `/gsd/discuss-milestone` creates this file at end of discussion
2. User runs `start a new chat` (safe now - context is persisted)
