...
export function currentLoadInfo(maxCpuFraction?: number): LoadInfo & { maxCpuFraction?: number } {
...
export async function waitForHeadroom(options: WaitForHeadroomOptions): Promise<void> {
  ...
  if (!(maxCpuFraction > 0 && maxCpuFraction <= 1)) {
    // Misconfiguration — do not block orchestration, just log once and return.
    logger?.warn({ maxCpuFraction }, 'resource-governor: invalid maxCpuFraction, skipping headroom check');
    return;
  }
  ...
  let info = currentLoadInfo(maxCpuFraction);
  if (info.cpuFraction <= maxCpuFraction) {
    logger?.debug(
      { load: info },
  ...
  logger?.warn(
    { load: info },
    'resource-governor: high CPU load detected, waiting for headroom',
  );

  // Slow-path: periodically poll until below threshold or timeout expires.
  while (info.cpuFraction > maxCpuFraction) {
    ...
    info = currentLoadInfo(maxCpuFraction);
  }
